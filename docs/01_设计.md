...
公开 API 入口以 **`speedvqa/__init__.py`** 的 `__all__` 为准（数据集、配置、模型构建等）；训练与推理更多通过 **engine / inference / export** 子模块或 **`speedvqa/examples/`** 演示。推荐导入方式见 **[02_开发.md](./02_开发.md)**。
...
统一产出目录约定见 **[04_部署.md](./04_部署.md)** 中「`runs/` 输出目录」一节。
...
（wheel 安装是否包含 `tests` / `examples` 见 **[02_开发.md](./02_开发.md)**。）
...
| 文档 / 路径 | 内容 |
|-------------|------|
| [02_开发.md](./02_开发.md) | 环境、测试、推荐导入与一键入口 |
| [03_使用.md](./03_使用.md) | 日常训练与预测操作 |
| [04_部署.md](./04_部署.md) | 导出、推理、`runs/` 约定与清理 |

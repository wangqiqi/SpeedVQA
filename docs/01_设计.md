...
| 方向 | 说明 |
|------|------|
| 模型 | 默认 **MobileNetV3** 视觉编码 + **DistilBERT** 文本编码 + **融合**（`model.fusion.method`：`concat` 默认；可选 `attention`、`bilinear`、**`film`**（文本对视觉 FiLM 调制后再拼接 MLP）、**`cross_attn`**（文本为 Q、视觉为 KV 的单层跨模态注意力））+ **MLP 分类头**；骨干可配置（如 `resnet50`）。**ONNX 导出**通过 **`SpeedVQAOnnxWrapper`** 暴露 `image` / `input_ids` / `attention_mask` 三输入与 **`logits`** 输出，与 **`ROIInferencer`** 侧约定一致。 |
...
公开 API 入口以 **`speedvqa/__init__.py`** 的 `__all__` 为准（数据集、配置、模型构建等）；训练与推理更多通过 **engine / inference / export** 子模块或 **`speedvqa/examples/`** 演示。环境与全栈操作、推荐导入见 **[02_使用.md](./02_使用.md)**。
...
统一产出目录约定见 **[02_使用.md](./02_使用.md)** 中「`runs/` 输出目录」一节。
...
（wheel 安装是否包含 `tests` / `examples` 见 **[02_使用.md](./02_使用.md)**「开发补充」。）
...
| 文档 / 路径 | 内容 |
|-------------|------|
| [02_使用.md](./02_使用.md) | 全栈流程：环境、数据、训练、导出、部署推理、`runs/`、测试与导入 |
